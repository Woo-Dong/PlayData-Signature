...
def get_plot_predict_fbprophet(real_df, pred_df): 

    source = ColumnDataSource(data=dict( 
        date=pred_df['date'], 
    #     real=confirmed_df['confirmed'], 
        predict=pred_df['yhat']
    ))
    ...
    tools = [pltm.PanTool(), pltm.LassoSelectTool(), pltm.BoxZoomTool(), 
            pltm.ResetTool(), pltm.WheelZoomTool(), hover]

    p = figure(plot_width=800, plot_height=500, x_axis_type='datetime', tools=tools)
    p.line('date', 'predict', source=source)

    p.circle(x=real_df['date'], y=real_df['confirmed'], 
        size=2, 
        legend='real',
        color='black')
    return json_item(p, 'predPlot')
